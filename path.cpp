...
public:
    bool hasPath(char* matrix, int rows, int cols, char* str)
    {
        ...
        for (int i = 0; i < rows; ++i)
        {
            ...
            {
                if(isHavepath(matrix,rows,cols,str,isOk,i,j))
                    return true;
            }
        }
        return false;
...
private:
    bool isHavepath(char* matrix, int rows, int cols, char* str,bool* isOk, int curx, int cury)
    {
        if(*str == '\0')
            return true;
        if(cury == cols)
        ...
        bool sign=isHavepath(matrix,rows,cols,str+1,isOk,curx-1,cury)
       	||isHavepath(matrix,rows,cols,str+1,isOk,curx+1,cury)
       	||isHavepath(matrix,rows,cols,str+1,isOk,curx,cury-1)
       	||isHavepath(matrix,rows,cols,str+1,isOk,curx,cury+1);
